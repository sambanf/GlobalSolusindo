...
namespace GlobalSolusindo.Api.Controllers
{
    public class RoleController : ApiControllerBase
    {
        ...
        public IHttpActionResult Create([FromBody]RoleDTO role)
        {
            ...
            using (var roleCreateHandler = new RoleCreateHandler(Db, ActiveUser, new RoleValidator(), new RoleFactory(Db, ActiveUser), new RoleQuery(Db), AccessControl))
            {
                using (var transaction = new TransactionScope())
                {
                    ...
                    {
                        SaveLog(ActiveUser.Username, createRole, JsonConvert.SerializeObject(role), "Success", "", "", JsonConvert.SerializeObject(saveResult.Model.Model));
                        return Ok(new SuccessResponse(saveResult.Model, saveResult.Message));
                    }
                    else
                    {
                        SaveLog(ActiveUser.Username, createRole, JsonConvert.SerializeObject(role), "Error", saveResult.Message, "", "");
        ...
        public IHttpActionResult Update([FromBody]RoleDTO role)
        {
            ...
            RoleDTO valueOd = new RoleDTO();
            valueOd = roleQuery.GetByPrimaryKey(role.Role_PK);
            ...
            using (var roleUpdateHandler = new RoleUpdateHandler(Db, ActiveUser, new RoleValidator(), new RoleFactory(Db, ActiveUser), new RoleQuery(Db), AccessControl))
            {
                using (var transaction = new TransactionScope())
                {
                    ...
                    if (saveResult.Success)
                    {
                        SaveLog(ActiveUser.Username, updateRole, JsonConvert.SerializeObject(role), "Success", "", JsonConvert.SerializeObject(valueOd), JsonConvert.SerializeObject(saveResult.Model.Model));
                        return Ok(new SuccessResponse(saveResult.Model, saveResult.Message));
                    }
                    else
                    {
                        SaveLog(ActiveUser.Username, updateRole, JsonConvert.SerializeObject(role), "Error", saveResult.Message, "", "");
                        return Ok(new ErrorResponse(ServiceStatusCode.ValidationError, saveResult.ValidationResult, saveResult.Message));
                    }
                }
            }
