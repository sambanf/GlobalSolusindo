...
{
    public class MappingUserToRoleGroupEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public MappingUserToRoleGroupEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("MappingUserToRoleGroup_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("MappingUserToRoleGroup_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                Type = "Button",
                Text = "Add Role",
                Tooltip = formState == EntryFormState.Create ? "Create mappingUserToRoleGroup" : "Update mappingUserToRoleGroup",
                Link = "/mappingUserToRoleGroup",
