...
{
    public class UserEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public UserEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("User_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("User_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save user.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create user" : "Update user",
                Link = "/user",
