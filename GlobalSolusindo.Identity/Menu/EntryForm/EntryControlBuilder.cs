...
{
    public class MenuEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public MenuEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("Menu_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("Menu_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save menu.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create menu" : "Update menu",
                Link = "/menu",
