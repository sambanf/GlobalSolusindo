...
{
    public class CostEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public CostEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("Cost_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("Cost_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save cost.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create cost" : "Update cost",
                Link = "/cost",
