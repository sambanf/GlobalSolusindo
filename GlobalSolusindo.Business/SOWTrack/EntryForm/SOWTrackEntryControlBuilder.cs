...
{
    public class SOWTrackEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public SOWTrackEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("SOWTrack_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("SOWTrack_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save sowTrack.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create sowTrack" : "Update sowTrack",
                Link = "/sowTrack",
