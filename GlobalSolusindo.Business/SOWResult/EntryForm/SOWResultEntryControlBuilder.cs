...
{
    public class SOWResultEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public SOWResultEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("SOWResult_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("SOWResult_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save sowResult.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create sowResult" : "Update sowResult",
                Link = "/sowResult",
