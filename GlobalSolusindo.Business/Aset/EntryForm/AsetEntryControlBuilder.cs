...
{
    public class AsetEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public AsetEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("Aset_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("Aset_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save aset.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create aset" : "Update aset",
                Link = "/aset",
