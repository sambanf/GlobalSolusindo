...
{
    public class DeliveryAreaEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public DeliveryAreaEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("DeliveryArea_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("DeliveryArea_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save deliveryArea.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create deliveryArea" : "Update deliveryArea",
                Link = "/deliveryArea",
