...
{
    public class OperatorEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public OperatorEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("Operator_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("Operator_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save _operator.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create _operator" : "Update _operator",
                Link = "/_operator",
