...
{
    public class IssueTypeEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public IssueTypeEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("IssueType_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("IssueType_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save issueType.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create issueType" : "Update issueType",
                Link = "/issueType",
