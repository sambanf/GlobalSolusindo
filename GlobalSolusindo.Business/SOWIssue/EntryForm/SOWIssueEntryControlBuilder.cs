...
{
    public class SOWIssueEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public SOWIssueEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("SOWIssue_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("SOWIssue_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save sowIssue.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create sowIssue" : "Update sowIssue",
                Link = "/sowIssue",
