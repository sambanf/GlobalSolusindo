...
{
    public class ProductEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public ProductEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("Product_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("Product_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save product.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create product" : "Update product",
                Link = "/product",
