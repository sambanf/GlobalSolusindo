...
{
    public class CheckInEntryControlBuilder : BuilderBase
    { 
        private AccessControl accessControl;

        public CheckInEntryControlBuilder(GlobalSolusindoDb db, tblM_User user, AccessControl accessControl) : base(db, user)
        { 
    ...
    class SaveButtonBuilder : BuilderBase
    {
        ...
        private bool GetEnabledState()
        {
            ...
            switch (formState)
            {
                case EntryFormState.Create:
                    enabled = accessControl.UserHasRole("CheckIn_Input");
                    break;
                case EntryFormState.Update:
                    enabled = accessControl.UserHasRole("CheckIn_Edit");
                    break;
                default:
                    break;
            }
        ...
        public Button GetButton()
        {
            return new Button()
            {
                ...
                CommandName = "Save",
                CommandDescription = "Save checkIn.",
                Type = "Button",
                Text = "Save",
                Tooltip = formState == EntryFormState.Create ? "Create checkIn" : "Update checkIn",
                Link = "/checkIn",
